<?php
    $page = 'Exercice 6'; //déclaration d'une variable pour afficher le numéro de l'exercice dans le html
    $message = '';
    $hide = '';
    include 'header.php'; //inclus le header et la navbar
    if (!empty($_POST['gender']) && !empty($_POST['lastname']) && !empty($_POST['firstname']))
    {
        $message = 'Bonjour, ' .trim(strip_tags($_POST['gender'])). ' ' .trim(strip_tags($_POST['lastname'])). ' ' .trim(strip_tags($_POST['firstname']));
    }
    else
    {
?>
        ...
        <div class="col-md-5 border border-dark p-5">
            ...
            <form method="post" action="ex6.php">               
                <div class="form-group">
                    <label class="mr-sm-2" for="inlineFormCustomSelect">Civilité</label>
